-- ===========================================================
-- DIAGNÓSTICO SIMPLIFICADO: ¿POR QUÉ NO SE RECONOCE COMO ADMIN?
-- ===========================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Obtener detalles
    SELECT status, club_id INTO v_admin_status, v_club_id
    FROM public.admins
    WHERE user_id = v_user_id;

    RAISE NOTICE '   Club ID: %', v_club_id;
    RAISE NOTICE '   Status: %', v_admin_status;
  END IF;

  -- Verificar status del admin
  IF v_admin_status != 'active' THEN
    RAISE NOTICE '❌ ADMIN NO ESTÁ ACTIVO';
    RAISE NOTICE '   Status actual: %', v_admin_status;
    RAISE NOTICE '   Solución: UPDATE admins SET status = ''active'' WHERE user_id = ''%'';', v_user_id;
    RETURN;
  ELSE
  ...
  IF v_club_name IS NULL THEN
    ...
    RETURN;
  ELSE
    RAISE NOTICE '✓ Club encontrado: %', v_club_name;
  END IF;

  -- Verificar función
  IF fn_current_admin_club_id() IS NULL THEN
    ...
    RETURN;
  ELSE
    RAISE NOTICE '✓ fn_current_admin_club_id() funciona: %', fn_current_admin_club_id();
  END IF;
